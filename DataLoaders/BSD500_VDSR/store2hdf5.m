function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename,filename1, data, labels, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename1, '/label', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
  ...
  if ~isempty(data)
    ...
    h5write(filename1, '/label', single(labels), startloc.lab, size(labels));  
  end
